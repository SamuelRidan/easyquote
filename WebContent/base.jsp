...
<html lang="pt-br">
	<head>
		<title> EasyQuote - Gerenciamento de Cotações e Contratos </title>
	 ...
	 <body data-spy="scroll" data-target=".subnav" data-offset="50" style="position: relative;">
		...
		<div id="fixed-top" class="navbar navbar-inverse navbar-fixed-top" >
			<div style="background: black; box-shadow: 5px 5px 10px grey;" >
				<div id="menu">
				    <ul class="menu">
				        <li><a href="<c:url value="/home/home"/>" class="last"><span>Menu</span></a>
				            <ul>
				            	<c:forEach var="modulo" items="${sessaoFuncionalidades.modulos}">
						        	<li><a href="" class="parent"><span>fora ${modulo.key}</span></a>
			                            <ul>
			                            	<c:forEach var="item" items="${modulo.value}">
			                                	<li>dentro <a href="<c:url value="/${item.codigo}"/>"><span>${item.nomeFuncionalidade}</span></a></li>
			                            	</c:forEach>
			                            </ul>
				                	</li>
				            	</c:forEach>
				    ...
				    <ul class="menu-operador" >
				    	<li><span> Logado como: ${sessaoOperador.operador.postoGraduacao} ${sessaoOperador.operador.login} &nbsp; </span>
				    	</li>
				    </ul>
